...
describe('먼저 바꾸고 나중에 확인한다', () => {
  ...
  // ⚠️ 이 판만 진짜 시계에 기대어 경주한다 — 낙관적 값이 보이는 창(아래 700ms) 안에서
  //    단언 하나를 하고, 그 뒤 서버 답을 기다린다. 판 135개가 CPU 를 나눠 쓰는 전체 실행에서는
  //    워커가 굶어 그 창을 통째로 놓치는 일이 간헐적으로 생긴다(단독 실행은 12/12 통과).
  //    커버리지 계측이 그 확률을 올린다. 한도를 넓히는 것으로는 안 잡혀서 재시도를 둔다.
  it('서버 답이 오면 서버 값으로 맞춰진다', { retry: 2, timeout: 20_000 }, async () => {
    const user = userEvent.setup();
    await login('jaehoon');
