...
describe('조건 안에서 Context 를 읽으면', () => {
  it('use 는 조건이 켜졌다 꺼져도 멀쩡하다', () => {
    ...
    expect(screen.getByTestId('a')).toHaveTextContent('안 읽음');

    turnOn();

    expect(screen.getByTestId('a')).toHaveTextContent('읽었음');
  });

  it('useContext 도 런타임에서는 안 터진다 — 규칙을 어겼는데 화면은 나온다', () => {
    const { turnOn } = renderToggling((show) => <ReadWithUseContext show={show} />);

    expect(screen.getByTestId('b')).toHaveTextContent('안 읽음');

    turnOn();

    expect(screen.getByTestId('b')).toHaveTextContent('읽었음');
  });

  it('앞에 진짜 훅이 살아 있어도 useContext 는 여전히 안 터진다', () => {
    const { turnOn } = renderToggling((show) => <ReadWithUseContextAfterState show={show} />);

    expect(screen.getByTestId('c')).toHaveTextContent('안 읽음(붙어있음)');

    turnOn();

    expect(screen.getByTestId('c')).toHaveTextContent('읽었음(붙어있음)');
  });
  ...
  it('같은 자리에 진짜 훅을 넣으면 그때는 터진다', () => {
    ...
    expect(screen.getByTestId('d')).toHaveTextContent('안 읽음(붙어있음)');
  ...
  it('진짜 훅 두 개 사이에 끼워도 뒤엣것의 순서가 안 밀린다 — 자리를 안 잡는다는 뜻', () => {
    ...
    expect(screen.getByTestId('e')).toHaveTextContent('앞/뒤');
