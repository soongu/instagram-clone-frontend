// C-9 Step 3 시점의 쪽지 화면을 그대로 얼려둔 것.
//
// Step 4 에서 살아 있는 쪽은 이력을 받아오고 캐시를 쓰도록 바뀐다.
// 그런데 "받아온 것이 없으면 어떻게 되나" 는 바뀌기 전 모습으로만 잴 수 있다.
// (C-6 의 c6-feed-section-before·C-7 의 c7-feed-section-before 와 같은 이유다.)
import { useEffect, useState } from 'react';
import { useParams } from 'react-router';
import type { Client } from '@stomp/stompjs';
import type { DirectMessage } from '../src/types/dm';
import { appendMessage, parseDirectMessage } from '../src/lib/dmEvents';
import { useConnectionStore } from '../src/stores/useConnectionStore';
import { useSessionStore } from '../src/stores/useSessionStore';
import { DM_QUEUE, DM_SEND, stompClient } from '../src/realtime/stompClient';
import { Button } from '../src/components/ui/button';

export function DmPageBeforeHistory({ client = stompClient }: { client?: Client }) {
